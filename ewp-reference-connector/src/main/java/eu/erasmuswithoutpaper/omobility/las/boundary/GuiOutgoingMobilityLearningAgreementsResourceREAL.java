...
public class GuiOutgoingMobilityLearningAgreementsResourceREAL {
    ...
    @GET
    @Path("get-partner_special")
    public Response getPartnerSpecial(@QueryParam("search_hei_id") String search_hei_id, @QueryParam("sending_hei_id") String sending_hei_id, @QueryParam("omobility_id") String omobility_id, @QueryParam("type") String type) {
        LOG.fine("get-partner: Hei searched: " + sending_hei_id);

        Map<String, String> heiUrls = registryClient.getOmobilityLasHeiUrls(search_hei_id);
        if (heiUrls == null || heiUrls.isEmpty()) {
            LOG.fine("get-partner: Hei not found: " + search_hei_id);
            return javax.ws.rs.core.Response.status(Response.Status.NOT_FOUND).build();
        }
        ...
        String heiUrl = heiUrls.get("get-url");
        if (heiUrl == null || heiUrl.isEmpty()) {
            LOG.fine("get-partner: Hei URL not found for: " + search_hei_id);
            return javax.ws.rs.core.Response.status(Response.Status.NOT_FOUND).build();
        }
